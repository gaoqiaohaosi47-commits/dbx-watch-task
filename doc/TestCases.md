...
| IT-10 | 異常 | 環境変数異常（WORKSPACE_LIST が不正JSON） | - | WORKSPACE_LIST に不正なJSON文字列を設定 | Functionを実行する | config.py の load_config_from_env() で例外発生。エラーログが出力される |
| IT-11 | 異常 | 環境変数欠落（DCE_ENDPOINT 未設定） | - | DCE_ENDPOINT を削除した環境変数 | Functionを実行する | config.py の load_config_from_env() で例外発生。エラーログが出力される |
| IT-12 | 異常 | 環境変数欠落（DCR_IMMUTABLE_ID 未設定） | - | DCR_IMMUTABLE_ID を削除した環境変数 | Functionを実行する | config.py の load_config_from_env() で例外発生。エラーログが出力される |
...
| IT-25 | 運用 | タイマースケジュール（定刻起動） | Functionデプロイ済み | 正常な設定 | 設定した実行間隔（30分）待機 | 設定間隔で定刻にFunctionが起動する |
...
| UT-05 | adapters/log_analytics_adapter.py | `_to_log_dict(record)` | 正常 | 成功レコードの dict 変換・TimeGenerated キー | 成功レコードのインスタンス | _to_log_dict(record) を呼び出す | "TimeGenerated"キーが存在し、値がISO8601形式。全フィールドがLA用キー名でdictに含まれる |
| UT-06 | adapters/log_analytics_adapter.py | `_to_log_dict(record)` | 正常 | 失敗レコードの Null フィールド変換 | 失敗レコードのインスタンス | _to_log_dict(record) を呼び出す | "endpoint_name", "endpoint_state", "endpoint_raw_data" がNullで含まれる |
| UT-07 | config.py | `load_config_from_env()` | 正常 | 全環境変数正常設定時の Config 生成 | 全環境変数が正しく設定 | load_config_from_env() を呼び出す | Configオブジェクトが生成され、全フィールドが正しい値 |
| UT-08 | config.py | `load_config_from_env()` | 異常 | DCE_ENDPOINT 未設定のバリデーション | DCE_ENDPOINT が未設定 | load_config_from_env() を呼び出す | ValueError または EnvironmentError が発生する |
| UT-09 | config.py | `load_config_from_env()` | 異常 | WORKSPACE_LIST 不正 JSON のバリデーション | WORKSPACE_LIST が不正なJSON | load_config_from_env() を呼び出す | ValueError が発生する |
| UT-10 | config.py | `load_config_from_env()` | 境界 | WORKSPACE_LIST 空配列時の Config 生成 | WORKSPACE_LIST が空配列 `[]` | load_config_from_env() を呼び出す | workspace_list が空リストのConfigオブジェクトが生成される |
...
| UT-16 | adapters/databricks_adapter.py | `DatabricksAdapter.fetch_endpoints()` | 正常 | 正常2エンドポイント取得・dict 変換 | requests.get をモック化、JSON `{"endpoints": [...]}` を返す | fetch_endpoints(workspace) を呼び出す | dict形式のエンドポイントリスト2件が返される |
| UT-17 | adapters/databricks_adapter.py | `DatabricksAdapter.fetch_endpoints()` | 異常 | HTTP エラーのサービス層への伝播 | requests.get が HTTP 4xx/5xx レスポンスを返す（raise_for_status で HTTPError） | fetch_endpoints(workspace) を呼び出す | requests.HTTPError に status_code 属性が付与されてサービス層に伝播する |
...
| UT-23 | adapters/databricks_adapter.py | `DatabricksAdapter.fetch_endpoints()` | 正常 | requests.get に workspace_url ベースの URL・timeout・Bearer ヘッダーが渡される | requests.get をモック化 | fetch_endpoints(workspace) を呼び出す | `requests.get("{workspace_url}/api/2.0/serving-endpoints", headers={"Authorization": "Bearer ..."}, timeout=HTTP_TIMEOUT_SECONDS)` が呼ばれる |
| UT-24 | adapters/databricks_adapter.py | `DatabricksAdapter.fetch_endpoints()` | 異常 | タイムアウトの TimeoutError への変換・伝播 | requests.get が requests.Timeout を送出する | fetch_endpoints(workspace) を呼び出す | TimeoutError が呼び出し元（サービス層）に伝播する |
...
| UT-29 | adapters/log_analytics_adapter.py | `_to_log_dict(record)` | 正常 | _to_log_dict() の全キー網羅確認 | 正常なEndpointRecordインスタンス | _to_log_dict(record) を呼び出す | 返り値のキーが期待する8キー（TimeGenerated, workspace_id, workspace_url, api_status_code, api_error_message, endpoint_name, endpoint_state, endpoint_raw_data）と完全一致 |
...
| UT-31 | config.py | `load_config_from_env()` | 異常 | WORKSPACE_LIST 未設定のバリデーション | WORKSPACE_LIST が未設定 | load_config_from_env() を呼び出す | ValueError が発生する |
| UT-32 | config.py | `load_config_from_env()` | 異常 | DCR_IMMUTABLE_ID 未設定のバリデーション | DCR_IMMUTABLE_ID が未設定 | load_config_from_env() を呼び出す | ValueError が発生する |
| UT-33 | config.py | `load_config_from_env()` | 異常 | DCR_STREAM_NAME 未設定のバリデーション | DCR_STREAM_NAME が未設定 | load_config_from_env() を呼び出す | ValueError が発生する |
| UT-34 | config.py | `load_config_from_env()` | 異常 | workspace_id フィールド欠落のバリデーション | WORKSPACE_LIST 要素に workspace_id フィールドが欠落 | load_config_from_env() を呼び出す | ValueError が発生する |
| UT-35 | config.py | `load_config_from_env()` | 異常 | monitor_enabled フィールド欠落のバリデーション | WORKSPACE_LIST 要素に monitor_enabled フィールドが欠落 | load_config_from_env() を呼び出す | ValueError が発生する |
...
| UT-39 | adapters/databricks_adapter.py | `DatabricksAdapter.fetch_endpoints()` | 正常 | requests.get の呼び出し URL が workspace_url に基づいて構築される | requests.get をモック化、JSON `{"endpoints": []}` を返す | fetch_endpoints(workspace) を呼び出す | requests.get が `{workspace_url}/api/2.0/serving-endpoints` の URL で呼ばれる |
...
| UT-43 | config.py | `load_config_from_env()` | 異常 | workspace_url フィールド欠落のバリデーション | WORKSPACE_LIST 要素に workspace_url フィールドが欠落 | load_config_from_env() を呼び出す | ValueError が発生する |
| UT-44 | config.py | `load_config_from_env()` | 正常 | 複数 WS の正常読み込み | WORKSPACE_LIST に2件（monitor_enabled=True/False）設定 | load_config_from_env() を呼び出す | workspace_list に2件が読み込まれ、各フィールドが正しい値になっている |
| UT-45 | adapters/databricks_adapter.py | `DatabricksAdapter.fetch_endpoints()` | 異常 | HTTP エラー発生時の status_code 属性付与 | requests.get が HTTP 403 レスポンスを返す（raise_for_status で HTTPError） | fetch_endpoints(workspace) を呼び出す | 伝播した HTTPError に `status_code=403` が付与されている |
| UT-46 | config.py | `load_config_from_env()` | 異常 | workspace_url が http:// の場合のバリデーション | WORKSPACE_LIST 要素の workspace_url が "http://" で始まる | load_config_from_env() を呼び出す | ValueError が発生する |
| UT-47 | config.py | `load_config_from_env()` | 異常 | DCE_ENDPOINT が http:// の場合のバリデーション | DCE_ENDPOINT が "http://" で始まる値 | load_config_from_env() を呼び出す | ValueError が発生する |
| UT-48 | config.py | `load_config_from_env()` | 正常 | workspace_url 末尾スラッシュの正規化 | workspace_url が末尾 "/" を含む | load_config_from_env() を呼び出す | 返された WorkspaceConfig の workspace_url から末尾 "/" が除去されている |
| UT-49 | adapters/log_analytics_adapter.py | `LogAnalyticsAdapter.send()` | 準正常 | 2 回失敗後 3 回目で送信成功 | upload() が最初 2 回例外を発生し、3 回目は成功 | send(records) を呼び出す（time.sleep をモック化） | 例外が送出されず、upload() が計 3 回呼ばれる |
| UT-50 | adapters/log_analytics_adapter.py | `LogAnalyticsAdapter.send()` | 異常 | MAX_RETRIES 回失敗後に例外が伝播 | upload() が常に例外を発生する | send(records) を呼び出す（time.sleep をモック化） | upload() が _MAX_RETRIES（3）回呼ばれ、最後の例外が呼び出し元に伝播する |
